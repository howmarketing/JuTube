...
const hiddenDir = path.join(homedir, ".JuTube");
...
// Checking for yt-dlp
let ytDlp;
let ytdlpPath = path.join(os.homedir(), ".JuTube", "ytdlp");

// ytdlp download path
let ytdlpDownloadPath;
if (os.platform() == "win32") {
	ytdlpDownloadPath = path.join(os.homedir(), ".JuTube", "ytdlp.exe");
} else {
	ytdlpDownloadPath = path.join(os.homedir(), ".JuTube", "ytdlp");
}
...
function afterSave(location, filename, progressId) {
	const notify = new Notification("JuTube", {
